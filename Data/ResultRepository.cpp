...
bool ResultRepository::open(const QString& dbPath)
{
    ...
    if (!db.open())
    {
        Logger::error(QString("Failed to open SQLite: %1").arg(db.lastError().text()));
        return false;
    }
...
bool ResultRepository::initSchema()
{
    ...
    if (!ok)
    {
        Logger::error(QString("Init schema failed: %1").arg(query.lastError().text()));
    }
    return ok;
}
...
bool ResultRepository::insertResult(const QString& cameraId, const AlgorithmResult& result)
{
    ...
    query.addBindValue(result.algorithmName);
    query.addBindValue(result.message);
    query.addBindValue(result.success ? 1 : 0);
    query.addBindValue(QDateTime::currentMSecsSinceEpoch());
    if (!query.exec())
    {
        Logger::warning(QString("Insert result failed: %1").arg(query.lastError().text()));
        return false;
    }
    return true;
}
...
QVector<DetectionRecord> ResultRepository::queryRecent(int limit) const
{
    ...
    if (!query.exec())
    {
        Logger::warning(QString("Query recent failed: %1").arg(query.lastError().text()));
        return records;
    }
